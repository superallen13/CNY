...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        for key in ["rollout_log_probs", "teacher_log_probs", "opd_action_mask"]:
            if key not in rollout_data:
                continue
            # opd_action_mask stays bool; logprob tensors cast to float32.
            target_dtype = torch.bool if key == "opd_action_mask" else torch.float32
            rollout_data[key] = [
                torch.tensor(
    ...
    def save_model(self, rollout_id: int, force_sync: bool = False) -> None:
        ...
        # async_train() auto-offloads (torch_memory_saver.pause) the model when
        # offload_train; it is therefore PAUSED here. Resume it before the dist save
        # so dist_checkpointing.save reads RESIDENT GPU memory. Otherwise
        # determine_global_metadata -> all_gather_object -> _object_to_tensor ->
        # .to(device) raises "CUDA error: invalid argument" on paused memory for any
        # model that exceeds the torch_memory_saver resident margin (~3B+; 1.7B fit
        # the margin so the save was silently fine). wake_up() also reloads the nccl
        # process groups the original code reloaded here.
        if self.args.offload_train:
            self.wake_up()
        ...
        # Re-offload to restore the post-train slept state the loop expects after
        # save() (offload_train() afterward is a no-op for the offload case; the next
        # update_weights reloads process groups itself). sleep() pauses memory +
        # destroys the nccl groups the original code destroyed here.
        if self.args.offload_train:
            self.sleep()
