...
def apply_opd_kl_to_advantages(
    ...
    When ``rollout_data["opd_action_mask"]`` is present (walker's structured-
    granularity path), reverse_kl is zeroed at positions where mask is False
    so the OPD KL contribution is structurally zero at non-action tokens —
    independent of any SGLang↔Megatron drift between student rollout-time and
    train-time log_probs.

    Args:
        args: Configuration containing `use_opd` and `opd_kl_coef`.
        rollout_data: Dict containing "teacher_log_probs" (required) and
            "opd_action_mask" (optional, walker-only).
    ...
    action_masks = rollout_data.get("opd_action_mask")
    if action_masks is not None:
        action_masks = [m.to(device=device, dtype=torch.bool) for m in action_masks]

    reverse_kls = []
    for i, adv in enumerate(advantages):
        reverse_kl = student_log_probs[i] - teacher_log_probs[i]
        if action_masks is not None:
            # Walker structured-granularity: zero out non-action positions so
            # SGLang↔Megatron drift at tag / <answer> / template positions
            # does not bleed into the advantage.
            reverse_kl = reverse_kl * action_masks[i].to(reverse_kl.dtype)
