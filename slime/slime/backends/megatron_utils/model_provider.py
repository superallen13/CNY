...
def _patch_megatron_bridge_cuda_graph_impl() -> None:
    """megatron-bridge 0.3.0rc0 has an internal field-rename inconsistency:
    ``gpt_provider.py:202`` references ``self.cuda_graph_impl`` but the
    GPTModelProvider class hierarchy (Qwen2/Qwen3 included) only carries
    ``cuda_graph_scope``. Without this shim, NVIDIA bridge-mode loads fail
    with::

        AttributeError: 'Qwen3ModelProvider' object has no attribute
            'cuda_graph_impl'. Did you mean: 'cuda_graph_scope'?

    We add a class-level fallback so attribute lookup resolves to "none",
    which disables the cuda-graph code paths (Megatron's own default).

    Drop once the SIF rebuilds with megatron-bridge >= 0.4.0.
    """
    try:
        from megatron.bridge.models.gpt_provider import GPTModelProvider
    except ImportError:
        return
    if not hasattr(GPTModelProvider, "cuda_graph_impl"):
        GPTModelProvider.cuda_graph_impl = "none"
...
def wrap_model_provider_with_freeze(original_provider, args):
    def wrapped_provider(
        ...
        **kwargs,
        ...
        _patch_megatron_bridge_cuda_graph_impl()
