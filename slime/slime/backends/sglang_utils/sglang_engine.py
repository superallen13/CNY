...
class SGLangEngine(RayActor):
    def __init__(
        ...
        # ROCm: scrub ROCR_VISIBLE_DEVICES (Ray's amd_gpu manager raises on it)
        # and ensure HIP_VISIBLE_DEVICES is set so torch.cuda finds the device.
    ...
    def _init_normal(self, server_args_dict):
        # walker patch: enable allow_auto_truncate so SGLang left-truncates
        # rare oversize prompts (walker hop-5 multi-turn can balloon past
        # context_length) instead of returning 400 → infinite retry → crash.
        server_args_dict["allow_auto_truncate"] = True
