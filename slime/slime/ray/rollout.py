...
try:  # sglang 0.5.7+ added cuda-graph memory type
    from sglang.srt.constants import GPU_MEMORY_TYPE_CUDA_GRAPH
except ImportError:  # 0.5.0rc0 (slime_rocm.sif) — fall back to kv-cache tag
...
class ServerGroup:
    ...
    def start_engines(self, port_cursors: dict[int, int] | None = None) -> tuple[list, dict[int, int]]:
        ...
        for i in range(len(self.all_engines)):
            ...
            # Forward HIP/CUDA visible devices so AMD actors see the SLURM-
            # allocated GPU even when Ray's runtime_env scrubs unset envs.
        ...
        if self.args.rollout_external:
            ...
            # Walker sweep 2026-05-06: when multiple slime instances share a
            # bunya physical host (apptainer uses host networking), they all
            # default to base_port=15000 and race on the same nccl_port range
            # (slime/ray/rollout.py:_get_current_node_ip_and_free_port grabs a
            # port that another instance binds before SGLang init completes).
            # Honor SLIME_BASE_PORT env so callers can give each instance a
            # disjoint window; fall back to 15000 (slime original default).
            _env_base = int(os.environ.get("SLIME_BASE_PORT", "15000"))
            base_port = max(port_cursors.values()) if port_cursors else _env_base
...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        if samples[0].opd_action_mask is not None:
            train_data["opd_action_mask"] = [sample.opd_action_mask for sample in samples]
    ...
    *,
    ...
    for rank, engine in rollout_engines:
        ...
        def get_addr_and_ports(engine, node_idx):
            ...
            start_port = node_port_cursor.get(node_idx, base_port + node_idx * 100)
...
def _start_router(args, *, has_pd_disaggregation: bool = False, force_new: bool = False) -> tuple[str, int]:
    ...
    # We will not use the health check from router. (Older sglang_router
    # versions in pinned containers don't accept this kwarg — set it only
    # if RouterArgs supports it. Walker doesn't depend on the health
    # check being explicitly disabled.)
